...
/**
 * @brief     Incremental forward Propagation of a layer with batch-wise from/to
 */
void LayerNode::incremental_forwarding(const std::vector<unsigned int> &from,
...
#ifdef DEBUG
  if (!run_context->validate(getNumInputConnections() == 0, !requireLabel()))
    throw std::runtime_error("Running incremental_forwarding vector version "
                             "layer " +
                             getName() + " invalidated the context.");
#endif
