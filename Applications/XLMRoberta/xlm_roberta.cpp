...
void printInputLayers(const std::vector<std::string> &params,
  ...
  for (const auto &param : params) {
    if (param.find("input_layers=") == 0) {
      std::cout << "[DEBUG] layver name --> " << layer_type
                << " input_layers: " << param.substr(13) << std::endl;
      break;
    } else if (param.find("input_layers") != std::string::npos &&
               param.find("=") != std::string::npos) {
      size_t pos = param.find("=");
      std::cout << "[DEBUG] layver name --> " << layer_type
                << " input_layers: " << param.substr(pos + 1) << std::endl;
      break;
    }
  }
}

void XLMRoberta::setupParameters(causallm::json &cfg,
  ...
  // MEMORY_SWAP = nntr_cfg.contains("fsu") ? nntr_cfg["fsu"].get<bool>() :
  // false; FSU_LOOKAHEAD = nntr_cfg.contains("fsu_lookahead")
  ...
  //                : DIM / NUM_HEADS; // default value is hidden_size /
  //                num_heads
...
void XLMRoberta::constructXLMRobertaPooler() {
  ...
  std::vector<std::string> pooler_dense_param = {
    withKey("name", "pooler_dense"), withKey("unit", DIM),
    withKey("input_layers",
            "encoder_" + std::to_string(NUM_LAYERS - 1) + "_output_layer_norm"),
  ...
  std::vector<std::string> activation_param = {
    ...
    withKey("input_layers", "pooler_dense"), withKey("activation", "tanh")};
...
void XLMRoberta::registerCustomLayers() {
  ...
  try {
    ...
    app_context->registerFactory(
      nntrainer::createLayer<xlmroberta::FullyConnectedSingleLayer>);
...
void XLMRoberta::constructXLMRobertaOutput(int layer_id,
  ...
  std::vector<std::string> output_param = {
    ...
    withKey("unit", DIM), withKey("input_layers", input_name),
    withKey("weight_initializer", "xavier_uniform"),
    withKey("bias_initializer", "zeros")};
  printInputLayers(output_param,
                   "encoder_" + std::to_string(layer_id) + "_output_dense");
  layers.push_back(createLayer("fully_connected", output_param));

  std::vector<std::string> tmp_add_params = {
    withKey("name", "encoder_" + std::to_string(layer_id) + "_output_tmp_add"),
    withKey("input_layers", "encoder_" + std::to_string(layer_id) +
                              "_attention_output_layer_norm,encoder_" +
                              std::to_string(layer_id) + "_output_dense")};
  printInputLayers(tmp_add_params,
...
void XLMRoberta::constructXLMRobertaIntermediate(int layer_id,
                                                 std::string input_name) {
  std::vector<LayerHandle> layers;
  ...
  std::vector<std::string> intermediate_param = {
    withKey("name",
            "encoder_" + std::to_string(layer_id) + "_intermediate_dense"),
    withKey("unit", INTERMEDIATE_SIZE), withKey("input_layers", input_name),
    withKey("weight_initializer", "xavier_uniform"),
    withKey("bias_initializer", "zeros")};
  printInputLayers(intermediate_param, "encoder_" + std::to_string(layer_id) +
                                         "_intermediate_dense");
  ...
  std::vector<std::string> activation_param = {
    withKey("name",
            "encoder_" + std::to_string(layer_id) + "_intermediate_act"),
    withKey("input_layers",
            "encoder_" + std::to_string(layer_id) + "_intermediate_dense"),
    withKey("activation", "gelu")};
  printInputLayers(activation_param,
                   "encoder_" + std::to_string(layer_id) + "_intermediate_act");
  layers.push_back(createLayer("activation", activation_param));
...
void XLMRoberta::constructXLMRobertaSelfOutput(int layer_id,
                                               std::string input_name) {
  std::vector<LayerHandle> layers;

  // std::vector<std::string> output_param = {
  //     withKey("name", "encoder_" + std::to_string(layer_id) +
  //     "_attention_output_dense"), withKey("unit", DIM),
  //     withKey("disable_bias", "false"),
  //     withKey("input_layers", input_name),
  //     withKey("weight_initializer", "ones")};
  // printInputLayers(output_param, "encoder_" + std::to_string(layer_id) +
  // "_attention_output_dense"); layers.push_back(createLayer("fully_connected",
  // output_param));
...
XLMRoberta::createAttention(const int layer_id, int seq_len, int n_heads,
  ...
  printInputLayers(o_params, O);
  layers.push_back(createLayer("fully_connected", o_params));

  return layers;
}

void XLMRoberta::constructXLMRobertaSelfAttention(int layer_id,
                                                  std::string input_name) {
  std::vector<LayerHandle> layers;
  ...
  std::vector<std::string> tmp_add_params = {
    withKey("name",
            "encoder_" + std::to_string(layer_id) + "_attention_tmp_add"),
    withKey("input_layers", input_name + ",encoder_" +
                              std::to_string(layer_id) + "_attention_self_wo")};
  printInputLayers(tmp_add_params, "encoder_" + std::to_string(layer_id) +
                                     "_attention_tmp_add");
  layers.push_back(createLayer("addition", tmp_add_params));
...
void XLMRoberta::constructXLMRobertaAttention(int layer_id,
                                              std::string input_name) {
...
void XLMRoberta::constructXLMRobertaLayer() {
  std::string input_name =
    "embedding_layer_norm"; // Start with embedding output
  ...
  for (int i = 0; i < NUM_LAYERS; ++i) {
    ...
    input_name =
      "encoder_" + std::to_string(i) + "_attention_output_layer_norm";
...
void XLMRoberta::constructXLMRobertaEncoder() { constructXLMRobertaLayer(); }
...
void XLMRoberta::constructXLMRobertaEmbeddings() {
  ...
  const std::string embedding_type =
    TIE_WORD_EMBEDDINGS ? "tie_word_embeddings" : "embedding";

  // Custom: position_ids
  std::vector<std::string> position_params = {
    "name=position", "weight_dtype=" + EMBEDDING_DTYPE,
    "out_dim=" + std::to_string(DIM), "input_layers=input0"};
  ...
  std::vector<std::string> token_type_params = {
    "name=token_type", "weight_dtype=" + EMBEDDING_DTYPE,
    "out_dim=" + std::to_string(DIM), "input_layers=input0"};
  ...
  printInputLayers(token_type_embedding_params,
                   "embedding_token_type_embedding");
  ...
  std::vector<std::string> addition0_params = {
    withKey("name", "embedding_addition0"),
    withKey("input_layers",
            "embedding_word_embedding,embedding_token_type_embedding")};
  ...
  std::vector<std::string> addition1_params = {
    ...
    withKey("input_layers",
            "embedding_addition0,embedding_position_embedding")};
...
std::string XLMRoberta::getLastComponent(const std::string &type) {
