...
// #define DEBUG
  ...
  for (int i = 0; i < batchSize; ++i) {
    std::cout << "____query" << i << ": " << ____query.getBatchSlice(i, 1);
  ...
  for (int i = 0; i < batchSize; ++i) {
    std::cout << "____key" << i << ": " << ____key.getBatchSlice(i, 1);
  ...
  for (int i = 0; i < batchSize; ++i) {
    std::cout << "____value" << i << ": " << ____value.getBatchSlice(i, 1);
...
void MHACoreLayer::compute_kcaches(
    ...
    if (mask_size == 1) {
      ...
      if (sequence_len == 1) {
        ...
        for (int n = 0; n < num_cache_head; ++n) {
          const __fp16 *in_ptr =
            in.getData<_FP16>() + n * group_size * head_dim;
          ...
          for (int tile_off = 0; tile_off < tile_count; ++tile_off) {
            futures.emplace_back(pool.submit_task([=]() {
              nntrainer::compute_kcaches(
                in_ptr, cache_ptr, out_ptr, num_rows, num_rows, num_cache_head,
                head_dim, group_size, tile_off, tile_size, local_window_size);
            }));
          }
        }
        for (auto &fut : futures)
  ...
  void MHACoreLayer::one_batch_incremental_forwarding(
    ...
    apply_rotary_emb_tensor_v2(query_step, query_step, head_dim, _from,
                               disable_rope ? true : false);
        ...
        for (j = from; j < to; j++) {
          if (j >= mask_from && j < mask_to)
            for (k = 0; k < num_heads_Q; k++) {
              ...
              // std::cout << "(JBD) painting mask,.. mask_" << mask_ <<
              // std::endl;
            }
        }
      }
    ...
    // std::cout << "(JBD) before compute K, query_step" << query_step <<
    // std::endl; std::cout << "(JBD) before compute K, mask_" << mask_ <<
    // std::endl; std::cout << "(JBD) before compute K, out_" << out_ <<
    // std::endl;
  ...
  void MHACoreLayer::apply_rotary_emb_tensor_v2(
    nntrainer::Tensor & in, nntrainer::Tensor & out, unsigned int dim,
  ...
  void MHACoreLayer::softmax_triangle(nntrainer::Tensor & qk_out, size_t row,
    ...
    if (qk_out.getDataType() == ml::train::TensorDim::DataType::FP32) {
      ...
      if (row == 1) {
        size_t start_row = 0;
        size_t end_row =
          from < local_window_size ? from + 1 : local_window_size;
        ...
        for (int i = 0; i < seq; ++i) {
          ...
          size_t end_row =
            calc_attn_index(from + i + 1) - calc_attn_index(from);
      ...
      if (row == 1) {
        ...
        for (int i = 0; i < seq; ++i) {
          ...
          size_t end_row =
            calc_attn_index(from + i + 1) - calc_attn_index(from);
          futures.push_back(pool.submit_task([=]() {
            nntrainer::softmax_row_inplace(qk_out_, start_row, end_row,
                                           num_head);
          }));
        }
        for (auto &fut : futures) {
  ...
  void MHACoreLayer::softmax_triangle(
    nntrainer::Tensor & qk_out, size_t row, size_t num_head, unsigned int from,
  ...
  void MHACoreLayer::softmax_full(
    nntrainer::Tensor & qk_out, size_t row, size_t num_head, unsigned int from,
    BS::thread_pool<> &pool, nntrainer::Tensor &sink_step) {
    if (qk_out.getDataType() == ml::train::TensorDim::DataType::FP32) {
      float *qk_out_ = qk_out.getData<float>();
  ...
  void MHACoreLayer::compute_fp16vcache_transposed(
    ...
    nntrainer::Tensor & output, int from, int num_cache_head, int gqa_size,
    int head_dim, int to, BS::thread_pool<> &pool) {
    ...
    if (in.getDataType() == ml::train::TensorDim::DataType::FP32) {
      if ((to - from) != 1) {
        ...
        int seq =
          (to - from) < local_window_size ? to - from : local_window_size;
      ...
      if ((to - from) != 1) {
        ...
        for (int n = 0; n < num_cache_head; ++n) {
          ...
          for (int chunk_off = 0; chunk_off < head_dim;
  ...
  void MHACoreLayer::setBatch(nntrainer::RunLayerContext & context,
    ...
    // context.updateTensor(tensor_idx[AttentionParams::attention_weight],
    // batch);
