 ...
 * @date	14 May 2020
 * @brief	This is Fully Connected Layer Class for Neural Network (single sequence variant)
...
FullyConnectedSingleLayer::FullyConnectedSingleLayer() :
  nntrainer::LayerImpl(),
  lora_scaling(1.0f),
  fc_props(nntrainer::props::Unit(), nntrainer::props::LoraRank(), nntrainer::props::LoraAlpha()),
  quantizer(nullptr) {
...
void FullyConnectedSingleLayer::finalize(nntrainer::InitLayerContext &context) {
  ...
  auto &bias_initializer = std::get<nntrainer::props::BiasInitializer>(*layer_impl_props);
  ...
  lora_scaling = (lora_rank && !std::get<nntrainer::props::LoraAlpha>(fc_props).empty())
  ...
  nntrainer::TensorDim bias_dim(
    ...
    nntrainer::TensorDim::TensorType(context.getFormat(), context.getActivationDataType()),
  ...
  nntrainer::TensorDim weight_dim(
    ...
    nntrainer::TensorDim::TensorType(context.getFormat(), context.getWeightDataType()),
  ...
  if (disable_bias.empty() || disable_bias.get() == false) {
    weight_idx[FCParams::bias] =
      context.requestWeight(bias_dim, bias_initializer, nntrainer::WeightRegularizer::NONE,
  ...
  if (lora_rank) {
    ...
    lora_idx[LORAParams::loraTmp] =
      context.requestTensor(loraTmp_dim, "hidden_tmp_lora", nntrainer::Initializer::NONE,
                            true, nntrainer::TensorLifespan::FORWARD_GRAD_LIFESPAN);

    lora_idx[LORAParams::loraOut] =
      context.requestTensor(loraOut_dim, "hidden_lora", nntrainer::Initializer::NONE, true,
  ...
  case ml::train::TensorDim::DataType::QINT16:
    quantizer =
      nntrainer::Quantization::createQuantizer(nntrainer::QScheme::PER_TENSOR_AFFINE);
    break;
  default:
    quantizer = nullptr;
    break;
...
void FullyConnectedSingleLayer::setProperty(const std::vector<std::string> &values) {
...
void FullyConnectedSingleLayer::forwarding(nntrainer::RunLayerContext &context, bool training) {
  ...
  if (!std::get<nntrainer::props::LoraRank>(fc_props).empty()) {
    ...
    nntrainer::Tensor &hidden_tmp_lora = context.getTensor(lora_idx[LORAParams::loraTmp]);
    nntrainer::Tensor &hidden_out_lora = context.getTensor(lora_idx[LORAParams::loraOut]);
...
void FullyConnectedSingleLayer::incremental_forwarding(nntrainer::RunLayerContext &context,
                                                       unsigned int from,
                                                       unsigned int to,
                                                       bool training) {
  nntrainer::Tensor &weight = context.getWeight(weight_idx[FCParams::weight]);
  ...
  for (unsigned int b = 0; b < hidden_.batch(); ++b) {
    nntrainer::Tensor input_step = input_.getSharedDataTensor(
      input_step_dim, b * input_dim.getFeatureLen() + from * input_dim.width(), true);
    nntrainer::Tensor hidden_step = hidden_.getSharedDataTensor(
      hidden_step_dim, b * hidden_dim.getFeatureLen() + from * hidden_dim.width(), true);
    ...
    if (!std::get<nntrainer::props::LoraRank>(fc_props).empty()) {
      ...
      nntrainer::Tensor hidden_tmp_lora_step =
        hidden_tmp_lora.getSharedDataTensor(
          ...
          b * hidden_tmp_lora.height() * hidden_tmp_lora.width() + from * hidden_tmp_lora.width(), true);
      nntrainer::Tensor hidden_out_lora_step =
        hidden_out_lora.getSharedDataTensor(
          hidden_out_lora_step_dim,
          b * hidden_out_lora.height() * hidden_out_lora.width() + from * hidden_out_lora.width(), true);
...
void FullyConnectedSingleLayer::calcDerivative(nntrainer::RunLayerContext &context) {
  nntrainer::Tensor &weight = context.getWeight(weight_idx[FCParams::weight]);

  const nntrainer::Tensor &derivative_ = context.getIncomingDerivative(SINGLE_INOUT_IDX);
...
void FullyConnectedSingleLayer::calcGradient(nntrainer::RunLayerContext &context) {
