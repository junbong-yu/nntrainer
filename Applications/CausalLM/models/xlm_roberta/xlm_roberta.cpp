...
namespace causallm
{
  ...
  void printInputLayers(const std::vector<std::string> &params, const std::string &layer_type)
  {
    ...
    for (const auto &param : params)
    {
      ...
      {
        std::cout << "[DEBUG] layver name --> " << layer_type << " input_layers: " << param.substr(13) << std::endl;
        break;
      }
  ...
  void XLMRoberta::constructXLMRobertaOutput(int layer_id, std::string input_name)
  {
    ...
    std::vector<std::string> tmp_add_params = {
        ...
        withKey("input_layers", "encoder_" + std::to_string(layer_id) + "_attention_output_layer_norm,encoder_" + std::to_string(layer_id) + "_output_dense")};
    printInputLayers(tmp_add_params, "encoder_" + std::to_string(layer_id) + "_output_tmp_add");
    ...
    std::vector<std::string> norm_param = {
        ...
        withKey("input_layers", "encoder_" + std::to_string(layer_id) + "_output_tmp_add")};
  ...
  void XLMRoberta::constructXLMRobertaSelfOutput(int layer_id, std::string input_name)
  {
    ...
    std::vector<std::string> layer_norm_params = {
        ...
        withKey("epsilon", "1e-5"),
        withKey("packed", "false"),
        withKey("input_layers", "encoder_" + std::to_string(layer_id) + "_attention_tmp_add")};
  ...
  void XLMRoberta::constructXLMRobertaEmbeddings()
  {
    ...
    std::vector<std::string> word_embedding_params = {
        ...
        "in_dim=" + std::to_string(((NUM_VOCAB + 32) / 32) * 32),  // round-up by 32 for quantization
        "weight_dtype=" + EMBEDDING_DTYPE,
        "out_dim=" + std::to_string(DIM),
        "input_layers=input0"};
    ...
    std::vector<std::string> position_embedding_params = {
        ...
        "in_dim=" + std::to_string(((MAX_POSITION_EMBEDDINGS + 32) / 32) * 32),  // round-up by 32 for quantization
        "weight_dtype=" + EMBEDDING_DTYPE,
        "out_dim=" + std::to_string(DIM),
    ...
    std::vector<std::string> token_type_embedding_params = {
        ...
        "in_dim=" + std::to_string(((VOCAB_TYPE_SIZE + 32) / 32) * 32), // round-up by 32 for quantization
        "weight_dtype=" + EMBEDDING_DTYPE,
        "out_dim=" + std::to_string(DIM),
