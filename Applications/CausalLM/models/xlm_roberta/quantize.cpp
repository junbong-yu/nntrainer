...
void registerAllModels() {
  ...
  factory.registerModel("XLMRobertaModel",
                        [](json cfg, json generation_cfg, json nntr_cfg) {
                          return std::make_unique<XLMRoberta>(
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    json generation_cfg =
      LoadJsonFile(model_path + "/generation_config.json");
    ...
    if (!target_config_path.empty()) {
      ...
      json target_cfg = LoadJsonFile(target_config_path);
    ...
    auto model = Factory::Instance().create(architecture, cfg,
